  ...
  return (
    <div className="flex flex-col h-full bg-card rounded-lg shadow-xl overflow-hidden border">
      <header className="p-4 border-b flex items-center space-x-3 bg-muted/50">
        ...
        <Avatar>
          <AvatarImage src={`https://placehold.co/100x100.png?text=${otherParticipant?.name.substring(0,2)}`} />
          <AvatarFallback>{otherParticipant?.name.substring(0,2) || <UserCircle2 />}</AvatarFallback>
        </Avatar>
        <div>
